...
wow = datasets.load_dataset('kilt_tasks')
#print(wow)
train_data = wow['train_wow']
valid_data = wow['validation_wow']
test_data = wow['test_wow']
#print(test_data[0])
#print(len(test_data))
#print(test_data[0]['input'])
#if len(test_data[0]['output']['answer'])>0 : print(test_data[0]['output']['answer'][0]) # the last 0 should remain constant being the answer in a list


# pack the list of utterances out into pandas DataFrame because the model takes this
listall_tr, listall_val, listall_te = [], [], []

conv_len_tr = len(train_data)
for conv in range(conv_len_tr):
  #list1 = train_data[conv]['input']
  #ans_str2 = train_data[conv]['output']['answer'][0]
  listall_tr.append(train_data[conv]['input'])
  if len(train_data[conv]['output']['answer'])>0 : listall_tr.append(train_data[conv]['output']['answer'][0]) # because some answers are empty

conv_len_va = len(valid_data)
for conv in range(conv_len_va):
  #list1 = valid_data['turns'][conv]['utterance']
  #listall_val.extend(list1)
  listall_val.append(valid_data[conv]['input'])
  if len(valid_data[conv]['output']['answer'])>0 : listall_val.append(valid_data[conv]['output']['answer'][0])

conv_len_te = len(test_data)
for conv in range(conv_len_te):
  #list1 = test_data['turns'][conv]['utterance']
  #listall_te.extend(list1)
  listall_te.append(test_data[conv]['input'])
  if len(test_data[conv]['output']['answer'])>0 : listall_te.append(test_data[conv]['output']['answer'][0])
